...
const defaultGastoState: GastoFormState = {
...
function previewDistribucion(form: GastoFormState) {
...
/**
 * Formulario de gasto reutilizable entre crear y editar.
 * El caller pasa `initial` (valores precargados) y `onSubmit` (función
 * que recibe el payload listo para insert/update en BD).
 */
export function GastoForm({
  ...
  initial?: Partial<GastoFormState>
  onSubmit: (payload: GastoInsert) => Promise<{ error: string | null }>
  submitLabel?: string
}) {
  ...
  function setField<K extends keyof GastoFormState>(key: K, value: GastoFormState[K]) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(false)
    if (error) return // el caller muestra el toast
  }
